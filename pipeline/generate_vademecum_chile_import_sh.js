...
const OUTPUT_FILE = path.join('/tmp', 'vademecum-chile-import.sh')
const DATABASE_URL = process.env.DATABASE_URL || 'postgresql://postgres:Alis%2A2019@localhost:5432/farmachile'
...
function sqlQuote(value) {
  ...
  const tag = '$vdata$'
  const text = String(value).replace(new RegExp(tag, 'g'), '$vdata_escape$')
  return `${tag}${text}${tag}`
}
...
function buildSql(rows, batchNumber) {
  const values = rows
    .map(row => `(${[
      sqlQuote(row.nombre),
  ...
  return `sql=$(cat <<'SQL_${batchNumber}'
...
function main() {
  ...
  script += `echo "📦 Registros preparados: ${rows.length}"\n`

  for (let i = 0, batchNumber = 1; i < rows.length; i += BATCH_SIZE, batchNumber += 1) {
    script += buildSql(rows.slice(i, i + BATCH_SIZE), batchNumber)
    script += `echo "✅ Importados ${Math.min(i + BATCH_SIZE, rows.length)}/${rows.length}"\n`
  }

  script += 'echo "\\n✔ Importación terminada"\n'

  fs.writeFileSync(OUTPUT_FILE, script, { mode: 0o755 })
